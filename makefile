...
DATABASES:=base.phylactery phylactery.sql
SQLITE3:=sqlite3
CC:=clang
CFLAGS:=-O3
...
SQLITE3CFLAGS:=-DSQLITE_OMIT_LOAD_EXTENSION -DSQLITE_THREADSAFE=0 -DSQLITE_DEFAULT_FOREIGN_KEYS=1 -DSQLITE_DEFAULT_MEMSTATUS=0 -DSQLITE_DISABLE_DIRSYNC -DSQLITE_TEMP_STORE=3 -DSQLITE_MAX_ATTACHED=4 -DSQLITE_DEFAULT_MMAP_SIZE=4194304 -DSQLITE_MAX_MMAP_SIZE=8388608 -DSQLITE_DEFAULT_LOCKING_MODE=1
...
all: databases phylactery

clean:
	rm -f $(DATABASES) game.phylactery $(SQLITEMATHLIB) phylactery sqlite3.zip
...
phylactery.sql: base.phylactery
	$(SQLITE3) $^ .dump > $@

game.phylactery: phylactery.sql
	rm -f $@*
	$(SQLITE3) $@ < phylactery.sql
...
phylactery: src/play.c src/sqlite3.c
